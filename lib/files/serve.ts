// =============================================================================
// FILE SERVING — the one place that turns stored file bytes into an HTTP body.
// -----------------------------------------------------------------------------
// WHY THIS FILE EXISTS
//
// Downloads were arriving as 0-byte files. Every previous fix chased the
// FILENAME (extension missing, quotes, mime guessing) — but the name was never
// the problem. The BODY was empty.
//
...
/**
 * The private Storage bucket that holds lead files (archived CVs).
 *
 * The id is still `whatsapp-media` because that is what the existing objects
 * live under — every archived CV on every lead. A Supabase bucket cannot be
 * renamed in place, so renaming it would mean copying every object and
 * repointing every `leads.cv_path`. Not worth the risk for a name nobody sees.
 * Change this constant only if you migrate the objects too.
 */
export const LEAD_FILES_BUCKET = 'whatsapp-media';
