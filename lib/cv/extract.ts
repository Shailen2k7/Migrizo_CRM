// =============================================================================
// CV EXTRACTION — a file goes in, a compact plain-text profile comes out.
// -----------------------------------------------------------------------------
// Server-side, and the file is never written anywhere. It is parsed in memory,
// reduced to text, and discarded. The ONLY thing that survives is the text on
// leads.profile_text — a few kilobytes per person, where a PDF would be a few
// hundred. That is the entire storage strategy, and it is why this module
// refuses to accept a storage path.
//
// Two jobs beyond parsing:
//
//   * Decide whether the thing is a CV at all. People send invoices, passports,
//     screenshots and offer letters down the same WhatsApp line. Writing an
//     invoice over somebody's CV would be worse than saving nothing.
//   * Keep it under a cap. A two-page CV is about 5KB of text; a fifteen-page
//     academic one can be 40KB. Over the cap we keep the parts a visa
//     assessment reads — experience, education, achievements, publications —
//     and drop the parts it does not.
//
// Shared word-for-word with Relay, which runs the same file on WhatsApp
// documents. Change one, change both.
// =============================================================================

import mammoth from 'mammoth';
import { extractText, getDocumentProxy } from 'unpdf';

/** Hard ceiling on what we will store, in bytes of UTF-8. */
...
export interface Extracted {
  kind: CvKind;
  /** Cleaned, cap-respecting text ready for leads.profile_text. */
  text: string;
  /** Bytes of the cleaned text BEFORE any condensing, for the UI to explain. */
  rawBytes: number;
  /** True when the text had to be condensed to fit under the cap. */
  condensed: boolean;
  /** Our confidence that this is a CV and not some other document. 0..1 */
  cvScore: number;
  /** What we could pull out for matching to a lead. */
...
// ── 2. tidy ─────────────────────────────────────────────────────────────────
// PDF extraction leaves page numbers, running headers, hyphenation breaks and
// walls of whitespace behind. None of it is content.

function clean(raw: string): string {
  return raw
    .replace(/\r\n?/g, '\n')
    .replace(/[ \t ]+/g, ' ')
    // "Page 3 of 7", "3 / 7", bare page numbers on their own line
    .replace(/^\s*(page\s+)?\d+(\s*(of|\/)\s*\d+)?\s*$/gim, '')
    // hyphenated line breaks: "interna-\ntional" -> "international"
    .replace(/(\w)-\n(\w)/g, '$1$2')
    // bullets of every flavour to one glyph
    .replace(/^[\s]*[•·▪◦●■\-–—*]\s*/gm, '• ')
    .replace(/\n{3,}/g, '\n\n')
    .split('\n').map((l) => l.trim()).join('\n')
    .trim();
...
// ── 3. is it a CV? ──────────────────────────────────────────────────────────
// Scored, not boolean, so the caller can pick its own threshold: the drop page
// can be lenient (a human is looking), the WhatsApp hook must be strict
// (nobody is).
...
// ── 4. contacts, for matching to a lead ─────────────────────────────────────

const EMAIL_RE = /[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/gi;
// Indian and international shapes; we normalise to the last ten digits later.
...
export function contactsOf(text: string): Extracted['contacts'] {
  ...
  // The name is almost always the first short line that is not contact info.
...
// ── 5. keep it under the cap ────────────────────────────────────────────────
// Sections a visa assessment reads, in the order it reads them. Anything else
// (hobbies, declarations, references, "I hereby certify") goes first.
...
/** Split on lines that look like section headings: short, no full stop, often upper-case. */
...
export function condense(text: string, cap = PROFILE_CAP_BYTES): { text: string; condensed: boolean } {
  ...
  // Pass 1: drop sections nobody assesses on.
  ...
  // Pass 2: keep only the sections we DO assess on, in reading order.
  const ranked = secs
  ...
  // Pass 3: trim from the end, on a line boundary, and say so.
 ...
 *
 * A PDF starts with "%PDF". A DOCX is a zip, so it starts with "PK\x03\x04".
 * Those two checks settle it before the filename or the declared MIME type get
 * a vote; both remain as the fallback for text files and anything unsniffable.
 */
...
export async function extractCv(
  buf: Buffer, filename: string, mime?: string | null,
): Promise<Extracted> {
