// =============================================================================
// CAMPAIGN TEMPLATE LIBRARY — 13 premium marketing emails for lead nurture.
// Visually rich, email-safe (inline CSS, table layout, no JS), on-brand.
// Each returns { subject, html } via build(firstName). Personalisation:
//   {{name}}  -> recipient first name  (greeting "Dear <name>,")
//   {{UNSUB}} -> unsubscribe link (injected by the send route)
// Two CTAs on every email: "Check Your Eligibility" (website) + WhatsApp.
// =============================================================================
...
const ELIGIBILITY = 'https://migrizo.com/check-eligibility';

function esc(s: string) {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

// ---- premium shell: logo + gold accent header, airy body, navy footer -------
...
<body style="margin:0;padding:0;background:${BG};font-family:'Segoe UI',Arial,Helvetica,sans-serif;">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BG};padding:28px 12px;"><tr><td align="center">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;box-shadow:0 6px 26px rgba(22,41,78,0.10);border-radius:16px;overflow:hidden;">
      ...
      <!-- Body -->
      <tr><td class="px" style="background:#ffffff;padding:28px 34px 34px;">${bodyHtml}</td></tr>
      <!-- Footer -->
      <tr><td style="background:${NAVY};padding:24px 34px;" align="left">
        ...
        <div style="font-size:11.5px;color:#C7D0E4;margin-top:5px;line-height:1.7;">Smart. Fast. Reliable Visas &middot; <a href="https://www.migrizo.com" style="color:${GOLD};text-decoration:none;">www.migrizo.com</a><br/><a href="mailto:info@migrizo.com" style="color:${GOLD};text-decoration:none;">info@migrizo.com</a> &middot; <a href="${WA}" style="color:${GOLD};text-decoration:none;">+44 7887 348822</a></div>
        <div style="font-size:10px;color:#8FA0C4;margin-top:14px;line-height:1.6;border-top:1px solid #2C446E;padding-top:12px;">You're receiving this because you enquired about UK visa options with Migrizo.<br/>{{UNSUB}}</div>
      </td></tr>
    </table>
  </td></tr></table>
</body></html>`;
...
// ---- building blocks --------------------------------------------------------
const eyebrow = (t: string) => `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 14px;"><tr><td style="background:#EEF2FF;border-radius:999px;padding:6px 13px;font-size:10.5px;font-weight:800;letter-spacing:1.4px;color:${BLUE};text-transform:uppercase;">${t}</td></tr></table>`;
const h1 = (t: string) => `<h1 class="h1" style="margin:0 0 16px;font-size:26px;line-height:1.28;color:${NAVY};font-weight:800;letter-spacing:-0.2px;">${t}</h1>`;
...
function callout(inner: string, tint = '#EEF2FF', border = BLUE) {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:10px 0 20px;"><tr>
    <td style="background:${tint};border-left:4px solid ${border};border-radius:12px;padding:18px 20px;font-size:15.5px;line-height:1.65;color:${NAVY};font-weight:600;">${inner}</td>
  </tr></table>`;
}
...
function statPair(a: string, al: string, b: string, bl: string) {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 20px;"><tr>
    <td width="50%" style="padding:5px;"><div style="background:#FDECEC;border-radius:14px;padding:22px 10px;text-align:center;"><div style="font-size:40px;font-weight:800;color:#DC2626;line-height:1;">${a}</div><div style="font-size:12px;color:${MUTED};margin-top:6px;">${al}</div></div></td>
    <td width="50%" style="padding:5px;"><div style="background:#E6F7EE;border-radius:14px;padding:22px 10px;text-align:center;"><div style="font-size:40px;font-weight:800;color:#10B981;line-height:1;">${b}</div><div style="font-size:12px;color:${MUTED};margin-top:6px;">${bl}</div></div></td>
  </tr></table>`;
}

function iconList(items: [string, string, string][]) {
  const rows = items.map(([icon, title, desc], i) => `<tr>
    <td width="52" valign="top" style="padding:${i === 0 ? '0' : '9px'} 0 9px;"><div style="width:42px;height:42px;border-radius:12px;background:linear-gradient(135deg,#EEF2FF,#E3E9FF);text-align:center;line-height:42px;font-size:20px;">${icon}</div></td>
    <td valign="top" style="padding:${i === 0 ? '2px' : '11px'} 0 9px 14px;"><div style="font-size:14.5px;font-weight:700;color:${NAVY};">${title}</div>${desc ? `<div style="font-size:13px;color:${MUTED};line-height:1.6;margin-top:3px;">${desc}</div>` : ''}</td>
  </tr>`).join('');
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:8px 0 16px;">${rows}</table>`;
}

const divider = () => `<div style="height:1px;background:#EAEDF5;margin:22px 0;"></div>`;
...
export const CAMPAIGN_TEMPLATES: CampaignTemplate[] = [
  {
    ...
    build: (n) => shell(
      ...
      para('Most people who never make it to the UK are not rejected. They simply never apply &mdash; held back by a quiet <i>"what if I\'m not good enough?"</i>') +
      para('Here is the honest maths of it:') +
      statPair('0%', 'chance if you never apply', '~50%', 'chance with a strong, well-built profile') +
      callout('Not applying isn\'t the safe option. It\'s the only option that <b>guarantees a "no".</b>') +
      para('The UK Global Talent Visa rewards people who present their achievements well &mdash; and that is exactly what we build with you. A quick, honest profile review tells you where you really stand, with no pressure.') +
      cta(),
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('The biggest myth about the UK Global Talent Visa is that it\'s only for famous people or geniuses. It isn\'t. It\'s for people doing strong, real work &mdash; who present it the right way.') +
      iconList([
      ...
      para('Let us look at your profile honestly and tell you where you stand &mdash; it takes minutes and costs nothing.') +
      cta(),
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('It\'s one of the UK\'s most flexible routes &mdash; for talented people in tech, science, academia, and the arts. No employer, no sponsor, no job offer required.') +
      iconList([
      ...
      divider() +
      para('It\'s a two-stage process &mdash; endorsement, then the visa itself &mdash; and Migrizo manages every step end to end.') +
      cta(),
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      iconList([
        ...
        ['✅', 'You want flexibility in the UK', 'To work, freelance, or build &mdash; without a sponsor.'],
      ]) +
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      iconList([
        ['1️⃣', 'Endorsement', 'An approved UK body reviews your evidence and confirms your talent. This is where profile-building matters most &mdash; and where we focus.'],
      ...
      divider() +
      para('Most people get stuck at stage one &mdash; not because they\'re unqualified, but because their evidence isn\'t mapped to what endorsers look for. That\'s exactly what we fix.') +
      cta(),
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('A software engineer &mdash; good career, no fame, no employer waiting in the UK &mdash; assumed the Global Talent Visa was out of reach.') +
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('The Global Talent Visa isn\'t just entry &mdash; it\'s the first step of a settled UK future.') +
      iconList([
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('You might be surprised who\'s getting through &mdash; not celebrities, but people who look a lot like you:') +
      iconList([
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('There\'s rarely a "perfect" moment &mdash; but waiting has a real, quiet cost:') +
      iconList([
  ...
  },
  {
    ...
    build: (n) => shell(
      ...
      para('Check your eligibility online, or message us on WhatsApp &mdash; whichever is easier.') +
      cta(),
