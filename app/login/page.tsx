...
function LoginForm() {
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    else { setSent(true); toast.success('Check your inbox for a magic link'); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-12" style={{ background: 'hsl(var(--bg))' }}>
      <div className="w-full max-w-[420px]">
        ...
        <div className="panel p-8">
          <h1 className="text-2xl font-bold tracking-tight mb-1.5">Sign in to Migrizo</h1>
          <p className="text-[13px] text-muted mb-6">Continue with Google or get a magic link by email</p>

          {sent ? (
            <div className="rounded-xl border border-border bg-surface-2 p-5 text-center">
              <div className="w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center" style={{ background: 'hsl(var(--green-soft))', color: '#10B981' }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
              </div>
              <div className="text-[14px] font-semibold mb-1">Magic link sent</div>
              <div className="text-[12.5px] text-muted">Check <span className="font-medium text-ink">{email}</span> to sign in</div>
              <button onClick={() => setSent(false)} className="mt-4 text-[12.5px] text-indigo-600 hover:underline">Use a different email</button>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleEmail}>
                ...
                <input type="email" required placeholder="you@company.com" value={email}
                  onChange={(e) => setEmail(e.target.value)} className="input mb-3" disabled={loading !== null} />
                <button type="submit" disabled={loading !== null || !email.trim()} className="btn btn-primary w-full justify-center py-3 disabled:opacity-50">
